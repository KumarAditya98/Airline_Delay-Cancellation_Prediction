...
```{r}
flight <- subset(flight,select=-c(op_carrier,origin,dest))
```   
...
Area under the curve is 0.6944
...
flight_small$response <- as.factor(if_else(flight_small$prob >=0.5, 1, 0))
flight_small$delayed_status <- as.factor(flight_small$delayed_status)

confusionMatrix(flight_small$delayed_status, flight_small$response)
```
