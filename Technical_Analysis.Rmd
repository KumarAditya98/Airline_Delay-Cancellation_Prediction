...
Southwest Airlines has the most flights and consequently, a higher number of delayed flights as well, but as our last plot suggests, it is not the worst, it is the 6th best and is above the threshold value by over 1%, so definitely not great compared to Delta Airlines.   
...
Lets look at the 20 most popular destination cities in the US.
```{r}
dummy4 <- flight %>% group_by(city_dest) %>% summarize(NOF = n()) 
